...
class Makefile
  ...
  def self.deep_read(initial_dir)
    dirs = [initial_dir]
    dirs.each_with_object([]) do |dir, memo|
      # puts "processing #{dir}"
      Dir.entries(dir).map{|f| dirs << File.join(dir, f) if f !~ /^\./ && File.directory?(File.join(Dir.pwd, dir, f))}
      memo << Dir.entries(dir).reject{|f| f =~ /^\./ || File.directory?(File.join(Dir.pwd, dir, f))}.collect do |f|
        yield(f, dir)
      end
    end
  end
  ...
  def self.generate
    ...
    includes = deep_read("lit/include") do |f, dir|
      (@includes_list ||= []) << f
      <<-EOS
#{get_name f}.hpp:
\tliterati tangle -o src/. #{File.join(dir, f)}
      EOS
    end
    ...
    if Dir.exists?("lit/mocks")
      mocks = deep_read("lit/mocks") do |f, dir|
        (@includes_list ||= []) << f
        <<-EOS
  #{get_name f}.hpp:
  \tliterati tangle -o src/. #{File.join(dir, f)}
    ...
    game_sources = deep_read("lit/src") do |f, dir|
      (@game_sources_list ||= []) << f
      <<-EOS
#{get_name f}.cpp:
\tliterati tangle -o src/. #{File.join(dir, f)}
    ...
    game_objects = deep_read("lit/src") do |f|
      (@game_objects_list ||= []) << f
      <<-EOS
...
\t$(CXX) $(IFLAGS) $(CXXFLAGS) -c -o $@ src/#{get_name f}.cpp
    ...
    if Dir.exists?("lit/test")
      test_sources = deep_read("lit/test") do |f, dir|
        (@test_sources_list || []) << f
        <<-EOS
  #{get_name f}.cpp:
  \tliterati tangle -o src/. #{File.join(dir, f)}
      ...
      test_objects = deep_read("lit/test") do |f|
        (@test_objects_list || []) << f
        <<-EOS
  ...
  \t$(CXX) $(IFLAGS) $(GTEST_IFLAGS) $(GMOCK_IFLAGS) $(CXXFLAGS) -c $(SRC_DIR)/#{get_name f}.cpp -o $@
      ...
      test_binaries = deep_read("lit/test") do |f|
